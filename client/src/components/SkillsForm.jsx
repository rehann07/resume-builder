...
const SkillsForm = ({ data, onChange }) => {
  ...
  return (
    <div className='space-y-4'>
        <div>
            <h3 className='flex items-center gap-2 text-lg font-semibold text-gray-500'>Skills</h3>
        ...
        data.length > 0 ? (
            <div className='flex flex-wrap gap-2'>
                {data.map((skill,index)=>(
                    <span key={index} className='flex items-center gap-1 px-3 py01 bg-blue-100 text-blue-800 rounded-full text-sm'>
                        {skill}
      ...
      <div>
        <p className='text-sm text-blue-800'><strong>Tip:</strong> Add 8-13 relevant skills. Include both technical skills (programming languages,tools) and soft skills (leadership,communication).</p>
      </div>
    </div>
  )
}
